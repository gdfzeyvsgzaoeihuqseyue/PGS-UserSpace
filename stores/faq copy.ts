  ...
  actions: {
    ...
    async voteUseful(id: string) {
      try {
        ...
        const response = await $fetch<any>(`/solution/vote-useful/${id}`, {
          baseURL,
          method: 'PATCH',
          credentials: 'include'
        })
        ...
        if (faq) {
          if (response && response.data) {
            faq.isUseful = response.data.isUseful
            faq.isUseless = response.data.isUseless
          } else {
    ...
    async voteUseless(id: string) {
      try {
        ...
        const response = await $fetch<any>(`/solution/vote-useless/${id}`, {
          baseURL,
          method: 'PATCH',
          credentials: 'include'
        })
